...
export default function Chatbot() {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://contentmanagementsystem-portfolio.onrender.com/api/chat', {
        method: 'POST',
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4">
      ...
      <div className="relative">
        {!isOpen && (
          <div className="absolute inset-0 bg-blue-500 rounded-full animate-[ping_3s_ease-in-out_infinite] opacity-20 pointer-events-none"></div>
        )}
        <button
          onClick={() => setIsOpen(!isOpen)}
          className={`relative w-14 h-14 rounded-full shadow-[0_0_15px_rgba(37,99,235,0.3)] flex items-center justify-center text-white transition-all duration-300 hover:scale-105 active:scale-95 ${
